...
public class AnswerFilter implements LogicFilter {
    ...
    public String filter(BehaviorMatter request) {
        ...
        //1、netty案例
        if (content.equals("netty案例")){
            return "Netty实战专题案例，Netty4.x案例从简单入门到应用实战，全篇37节优秀案例+实战源码[基础篇(12)、中级篇(13)、高级篇(3章+)、源码分析篇(6)]，以上章节全部完成并不断持续更新中。\n" +
                    "\n" +
                    "源码地址；（Netty4.1案例）\n" +
        ...
        //3、用Java实现jvm源码
        if (content.equals("用Java实现jvm源码")){
        ...
        //4、基于JavaAgent的全链路监控案例
        if (content.equals("基于JavaAgent的全链路监控案例")){
            return "本专题主要介绍如何通过java代码来实现JVM的基础功能（搜索解析class文件、字节码命令、运行时数据区等），从而让java程序员通过最熟知的java程序，学习JVM是如何将java程序一步步跑起来的。\n" +
                    "\n" +
                    "源码地址\n" +
                    "https://github.com/fuzhengwei/itstack-demo-jvm\n" +
        ...
        //5、iot-gateway案例源码
        if (content.equals("iot-gateway案例源码")){
        ...
        //7、SpringCloud专题
        if (content.equals("SpringCloud专题")){
        ...
        //8、微信公众号与博客打通
        if(content.equals("itstack-ark-wx")){
